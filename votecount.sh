...
then
    ...
    poll_string="<label for=\"1\">To</label>	<br/> {{ range \$key, \$value := . }} <input type=\"radio\" name=\"1\" value=\"{{\$key}}\" checked> {{\$key}} <img src=\"/res/photos/{{\$value}}\" width=\"60px\" height=\"60px\"><br> {{ end }}"
    poll_string_final=""
    for i in $(seq 1 $1); do 
